...
function dropLiquid(inputState){
    var {time, ...state} = inputState;
    state.time = 0;
    socket.send("cgSg 1");
...
function leftHandHandBackTube(inputState){
    var {time, ...state} = inputState;
    state.time = 0;
    socket.send("cgSg 1");
    socket.send("rtSvTwo 0 94 1 1");
...
function rightHandHandBackTube(inputState){
    var {time, ...state} = inputState;
    state.time = 0;
    socket.send("cgSg 1");
    socket.send("mvMvBs -26.5 1 2");
...
function rightHandReachOutGetHCl(inputState){
    var {time, ...state} = inputState;
    state.time = 0;
    socket.send("cgSg 1");
    socket.send("rtSvTwo 1 88 1 1");
...
function rightHandFinishGetHCl(inputState){
    var {time, ...state} = inputState;
    state.time = 0;
    socket.send("mvAm 1 90 4 5");
    state.time += 1;
...
function rightHandReachOutGetCaCO3(inputState){
    var {time, ...state} = inputState;
    state.time = 0;
    socket.send("cgSg 1");
    socket.send("rtSvTwo 1 88 1 1");
...
function rightHandFinishGetCaCO3Watch(inputState){
    var {time, ...state} = inputState;
    state.time = 0;
...
function rightHandHandBackBeaker(inputState){
    var {time, ...state} = inputState;
    state.time = 0;
    socket.send("cgSg 1");
...
function rightHandStartObservePipe(inputState){
    var {time, ...state} = inputState;
    state.time = 0;
    socket.send("cgSg 1");
...
function rightHandObservePipe(inputState){
    var {time, ...state} = inputState;
    state.time = 0;
    socket.send("lfLfBs 1 -45");
    state.time += Math.abs(state.rightLift - (-45)) * liftSpeed;
    state.rightLift = (-45);
    state.time += timeDelta;
    return state;
}

function rightHandStopObservePipe(inputState){
    var {time, ...state} = inputState;
    state.time = 0;
    socket.send("cgSg 1");
...
function rotatePipeValve(inputState){
    var {time, ...state} = inputState;
    state.time = 0;
    socket.send("cgBr 0");
    state.time += timeDelta;
...
function leftHandHandErlenmeyerFlask(inputState){
    var {time, ...state} = inputState;
    state.time = 0;
    socket.send("cgSg 1");
    socket.send("rtSvTwo 0 94 1 1");
